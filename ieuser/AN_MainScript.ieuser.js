...
AN.execFunc = function(booIsFirstTime, jDoc)
{
	...
	if(booIsFirstTime)
	{
		$.each(AN.comp, function(strFnName)
		{
			if(((this.page[0] == 'all' && AN.data.strCurPage != 'special') || $.inArray(AN.data.strCurPage, this.page) != -1) && !this.disabled)
			{
				var numTime = $.time();
				this.once();
				AN.temp.benchmark.push([strFnName, ($.time() - numTime)]);
			}
		});
	}
		...
		if(((AN.data.settings1['all' + this.id] && AN.data.strCurPage != 'special')  || AN.data.settings1[AN.data.strCurPage + this.id]) && !this.disabled)
		{
			if((booIsFirstTime && this.once) || this.infinite)
			{
				try
				{
					...
					AN.temp.benchmark.push([this.disp, ($.time() - numTime)]);
				}
				catch(err)
				{
	...
	if(AN.temp.aToExec.length) AN.execFunc(false, $(AN.temp.aToExec));
	else
	{
		AN.temp.jDoc = $();
		AN.temp.aTopicRows = null;
	}
};
			...
			AN.func.getPage = function(bIsManual)
			{
				...
				if(!$('select[name=page]:first :selected').next().length)
				{
					...
					$.get(AN.shared.getURL(nCurPageNo), function(sHTML)
					{
						...
						bHasNewPage = $('select[name=page]:first').children().length != jDoc.find('select[name=page]:first').children().length,
						jReplies = jDoc.find('.repliers'),
						bHasNewReplies;

						if(!bIsRepliesMax)
						{
							if(AN.data.jReplies[nCurPageNo].length < jReplies.length) bHasNewReplies = true;
						}
						...
						else if(bHasNewReplies)
						{
							$('strong:first').text(sNewStrong);

							if(bHasNewReplies || bHasNewPage)
							{
		...
		once: function()
		{
			...
			AN.func.addButtons = function()
			{
				...
				$('<div id="AN_divSticky" style="padding: 0 0 0 5px" />').appendTo('#AN_divTopicFunctions').click(function()
				{
					...
					{
						aSticked.push(sData);
						var jTr = AN.func.stickyTopic(true, sData);
			...
			{
				var jTr = AN.func.stickyTopic(true, this);
				AN.temp.aToExec.push(jTr.get(0));
			});
		},
		infinite: function()
		{
			if(AN.temp.jDoc.filter('.AN_trSticky').length) return;
...
AN.temp.benchmark.push(['Total', ($.time() - nStart)]);
