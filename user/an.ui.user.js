...
/*
 * Known issues:
 * Chrome doesn't arrange properties inside an object in add order, causing wrong order for fnType in the panels
 */
...
'6464e397-dfea-477f-9706-025ec439e810':
{
	...
	once: function()
	{
		...
		.an-box { display:none; position: fixed; left: 50%; top: 50%; z-index: 2; border-width: 1px; } \
		...
		\
		#an-backlayer { display: none; opacity: 0; -ms-filter: "alpha(opacity=0)"; z-index: 1; position: fixed; left: 0; top: 0; width: 100%; height: 100%; background: gray; } \
		\
...
'0868eb64-9631-42dd-8b5c-02f11a8c9a48':
{
	...
	once: function()
	{
		...
		AN.shared.log = function(sLog)
		{
			...
			$($.sprintf('<li>%s:%s %s</li>', hour, min, sLog)).prependTo('#an-log-content').slideDown('slow');
		};
